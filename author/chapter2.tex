...
Finally, since the approximation step entails up to $\frac{2 M}{p-1}$ pluckings (each plucking decreases the number of sets by $p-1$, and there are no more than $2M$ sets when we start), the lemma holds for the $\lor$ approximation step,  because (recall the total number of $(k-1)$-colouring is  $(k-1)^n$) 
$$
...
if \Cref{eq:or_case_1} holds, then \Cref{eq:or_case_2} cannot.\footnote{When the core is empty, we get that the empty set is one of the sets in $\text{pluck}(\mathcal{X} \cup \mathcal{Y})$, which means that one of the AND gates in $\text{APX}(\text{pluck}(\mathcal{X} \cup \mathcal{Y}))$ is empty, namely the constant truth value 1.}
\medskip 
