...
function we can compute it by a monotone Boolean circuit. But the question remains whether we can compute CLIQUE$(n, k)$ with a small monotone circuit. 
...
\begin{figure}
\begin{tikzpicture}[scale=1, every node/.style={scale=1}, ultra thick, blue]\label{fig:clique-naive}
    % Define AND gates (larger and bold)
    \node (and1) at (-3,0) {\textbf{\Huge $\land$}};
    \node (and2) at (3,0) {\textbf{\Huge $\land$}};
    
    % Define OR gate (larger and bold)
    \node (or) at (0,2) {\textbf{\Huge $\lor$}};
    
    % Connect AND gates to OR gate with ultra thick blue lines
    \draw[ultra thick, blue] (and1) -- (or);
    \draw[ultra thick, blue] (and2) -- (or);
    
    % Add horizontal ellipsis between AND gates and OR gate
    \node (dots) at (0,0.6) {\Huge $\cdots$};
    
    % Define inputs to AND gates
    \node (x1) at (-5,-1) {$x_{1,2}$};
    \node (x2) at (-4,-1) {$x_{1,3}$};
    \node (x3) at (-3,-1) {$x_{k-1}$};
    \node (x4) at (-2,-1) {$x_{2,1}$};
    \node (x5) at (-1,-1) {$x_{2,3}$};
    
    \node (y1) at (1,-1) {$x_{10,12}$};
    \node (y2) at (2,-1) {$x_{10,23}$};
    \node (y3) at (3,-1) {$x_{7,1}$};
    \node (y4) at (4,-1) {$x_{7,100}$};
    
    % Connect inputs to AND gates with ultra thick blue lines
    \draw[ultra thick, blue] (x1) -- (and1);
    \draw[ultra thick, blue] (x2) -- (and1);
    \draw[ultra thick, blue] (x3) -- (and1);
    \draw[ultra thick, blue] (x4) -- (and1);
    \draw[ultra thick, blue] (x5) -- (and1);
    
    \draw[ultra thick, blue] (y1) -- (and2);
    \draw[ultra thick, blue] (y2) -- (and2);
    \draw[ultra thick, blue] (y3) -- (and2);
    \draw[ultra thick, blue] (y4) -- (and2);
    %
    % Labels for sets S_1 and S_{(n/k)}
    \node[red] at (-3,-2) {$S_1$};
    \node[red] at (3,-2) {$S_{n\choose k}$};
...
The rest of this chapter aims to prove this theorem. %\textit{Some of our exposition is taken from Papadimitriou's %textbook} \cite{Pap94}.
...
We shall describe a way of approximating any \textbf{monotone} circuit for $\operatorname{CLIQUE}({n}, {k})$ by an approximator, namely a big OR of cliques, as follows.
 

\begin{tcolorbox}[colframe=white, colback=blue!5, boxrule=0mm, sharp corners]
\textbf{Approximation Method}:
The upshot of the method is the following: \emph{by  way of contradiction,} consider a purported small monotone circuit computing  $\operatorname{CLIQUE}({n}, {k})$, and show by induction, gradually progressing from the input nodes towards the output node, that the function computed at every gate can be well approximated by a small approximator. Then, we show that if the number of nodes in the circuit is small the output gate is also well approximated by a small approximator. Now, use an auxiliary argument to show that there is no small approximator  approximating  well $\operatorname{CLIQUE}({n}, {k})$.
\end{tcolorbox}
% =======
...
\item  Given a monotone circuit $C$, we shall construct an approximator 
...
\item We show that every approximator ${\APPX}({X}_1, \ldots, {X_m})$ ($|{X}_i| \leq l$ for some $l$, for all $i=1, \ldots, m$ ), ought to make \textit{exponentially many errors} on the function $\operatorname{CLIQUE}(n, k)$. From \ref{it:approximation-b} above we conclude that there exists no circuit $C$ with a small number of gates, if $\ell$ and $m$ are not too big. 
\end{enumerate}
...
\subsection{The Input Set: Extreme Graphs, False Positives and  Negatives}
%___________________________________________________________________
...
\para{Extreme Graphs}
We  restrict attention to only a subsets of accept and reject instances, which we call \emph{extreme graphs}. This is sufficient for the proof (and simplifies it). 
...
\begin{tikzpicture}[scale=0.6] % Scale down the whole picture
    % Draw the large outer set G
    \draw[thick, blue] (0,0) ellipse (5cm and 3cm);
    \node[blue, above left] at (-4,2.3) {\Large $G$};
    
    % Label n = 12
    \node[green, left] at (-4,0) {\Large $n=12$};
    
    % Inner clique of k = 6 at the center
    \def\radius{1.2}
    \foreach \i in {0,1,2,3,4,5} {
        \node (v\i) at ({\radius*cos(60*\i)},{\radius*sin(60*\i)}) [blue,fill, circle, inner sep=3pt] {};
    }
    
    % Connect the clique with red edges
    \foreach \i in {0,1,2,3,4,5} {
        \foreach \j in {0,1,2,3,4,5} {
            \ifnum \i<\j
                \draw[ultra thick, red] (v\i) -- (v\j);
            \fi
        }
    }
    
    % Label k = 6
    \node[green, below] at (0,-1.7) {\Large $k=6$};
    
    % Outer scattered nodes randomly placed
    \foreach \pos in {(2,2), (-2.5,1.8), (3,-1), (-3,-2), (1.5,-2.5), (-1,2.2)} {
        \fill[blue] \pos circle (3pt);
    }
...
Input nodes $x_{ij}$. Define the approximator of $x_{ij}$  to be precisely $x_{ij}$, that is $\gapx(x_{ij}):=x_{ij}$.
...
An \emph{event} is a set $E\subseteq\Omega$ of atomic outcomes. The probability of an event $E$ is naturally defined as $\Sigma_{e\in E}\Pr[e]$. 
...
Thus, the number of \emph{false negatives} $\operatorname{\APPX}\left(X_1, \ldots, X_m\right)$ has in total is the number of all possible positive instances, namely all possible $k$-cliques, which is $\binom{n}{k}$.
...
in the sense that they introduce at most $M^2 \cdot \frac{(k-1)^n}{2^p}$ new false positives and at most $M^2 \cdot\binom{n-l-1}{k-l-1}$  new false negatives (for both the $\lor$ and $\land$ cases).\end{tcolorbox}
...
\case 2 Otherwise, intuitively there is enough intersection to find an element that is very popular; namely, the element appears in a lot of sets in $Z$. Hence, let $E$ be the \emph{union} of all sets in $D$.
...
Since $Z$ has more than $M$ sets by assumption, and each set intersects some element of $E$, there exists an element $d \in E$ that intersects $>\frac{M}{(p-1)\cd l}=(p-1)^{l-1} \cdot(l-1)!$ sets in $Z$. To see why that is so consider the following.

If a set $E$ intersects all sets in a family of sets $X_1, . ., X_M$ then there is an element in $E$ that appears in $\geq \frac{M}{|E|}$ sets $X_i$.
Otherwise, each element in $E$ appears in $<\frac{M}{|E|}$ sets $X_i$. Thus, $E$ intersects $<|E| \cdot \frac{M}{|E|}=M$ sets $X_i$, which is a contradiction to the assumption. Another way to look at this is through the pigeonhole principle: if $E$ intersects each set $X_i$, then there is a mapping from each of the $M$ sets $X_i$ to a (single) element of $E$ that occurs also in $X_i$ (in case there are several such elements choose one arbitrarily). Thus, there must be an element of $E$ with at least $\frac{M}{|E|}$ sets $X_i$ that maps to it. See  \Cref{fig:sunflower-simple-counting}  
...
\begin{tikzpicture}[
  ...
  transform shape,
  font=\small,
  set/.style={draw, rounded corners, inner sep=6pt, minimum width=16mm, minimum height=10mm},
  elem/.style={draw, circle, inner sep=1.6pt},
  arrow/.style={->, thick}
  ]

% Left: sets X_1, X_2, X_3, ..., X_M
\node[set] (X1) {$X_{1}$};
\node[set, below=7mm of X1] (X2) {$X_{2}$};
\node[set, below=7mm of X2] (X3) {$X_{3}$};
\node[below=10mm of X3] (Xdots) {$\vdots$};
\node[set, below=10mm of Xdots] (XM) {$X_{M}$};

% Right: elements of E stacked vertically
\node[elem, right=45mm of X1] (e1) {$e_{1}$};
\node[elem, below=6mm of e1] (e2) {$e_{2}$};
%\node[elem, below=6mm of e2] (e3) {$e_{3}$};
\node[below=6mm of e2] (edots) {$\vdots$};
\node[elem, below=6mm of edots] (ek) {$e_{k}$};

% Box around E and label
\node[draw, rounded corners, fit=(e1)(ek), inner sep=10mm] (Ebox) {};
\node[above=2mm of Ebox.north] {$E$};

% Arrows (no labels), with X_2 -> e_3 and X_3 -> e_2
\draw[arrow] (X1.east) -- (e1.west);
\draw[arrow] (X2.east) -- (e2.west);
\draw[arrow] (X3.east) -- (e2.west);
\draw[arrow] (XM.east) -- (ek.west);

\end{tikzpicture}
\caption{}\label{fig:sunflower-simple-counting}
\end{figure}
...
\begin{description}
\item[The approximator for $g_1\lor g_2$:] Assume that $\gapx(g_1)=\APPX(\cX)$ and $\gapx(g_2)=\operatorname{\APPX}(\cY)$. Define 
$$\gapx(g_1\lor g_2):=\APPX(\operatorname{pluck}(\cX \cup \cY)).$$

\item[The approximator for $g_1\land g_2$:]
...
$\operatorname{\APPX}\left(Y_1, \ldots, Y_{m'}\right)$ and consider a false positive introduced by
...
\quad \text{ and } ~~~\operatorname{\APPX}\left(Y_1, \ldots, Y_{m'}\right)(G)=0
\end{equation}
while 
...
replace the sunflower $\left\{Z_1, \ldots, Z_p\right\}$ by its core denoted $Z$ (see \Cref{fig:pluck-stage}).
...
\begin{equation}\label{eq:monotone-crucial-inequality-for-petal-disjointness:1}
\operatorname{Pr}
...
Now we use crucially the fact that the petals $Z_i$'s do not have common nodes, except those in the core $Z$. This allows us to conclude that the events $R(Z_i)$, for $i\in[p]$,  that a random $(k-1)$-colouring colors two nodes with the same colour in petal $Z_i$ given $\neg R(Z)$ are \emph{independent} and hence the probability that \emph{each} petal contains a pair with the same colours decrease exponentially in the number of petals $p$; namely, the right hand side of \Cref{eq:monotone-crucial-inequality-for-petal-disjointness:1} equals
\begin{align}
...
& \leq \frac{1}{2^p}
\end{align}
(because as we have seen before in \Cref{sec:Concluding-monotone-lower-bound-str-ind}, 
$\operatorname{Pr}[R(X)] \leq \frac{1}{2}$, for $|X|\le \ell$).


Finally, since the approximation step entails up to $\frac{2 M}{p-1}$ pluckings (each plucking decreases the number of sets by $p-1$, and there are no more than $2M$ sets when we start), the lemma holds for the $\lor$ approximation step,  because (recall the total number of $(k-1)$-colouring is  $(k-1)^n$) using the union bound
$$
...
The proof of the lemma is complete,  because in both cases we introduced in total at most $ M^2 \cdot \frac{(k-1)^n}{2^p}$ false positives.
...
\case{$\lor$} Consider the following false negative: a $k$-clique $G = (V, E)$ such that 
...
if \Cref{eq:or_case_1} holds, then \Cref{eq:or_case_2} cannot.\footnote{When the core is empty, we get that the empty set is one of the sets in $\text{pluck}(\mathcal{X} \cup \mathcal{Y})$, which means that one of the AND gates in $\text{APX}(\text{pluck}(\mathcal{X} \cup \mathcal{Y}))$ is empty, namely the constant truth value 1. Note that in this case the whole approximator is simply the constant 1.}
\medskip 
