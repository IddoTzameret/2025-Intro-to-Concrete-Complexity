...
We can turn the $t$-CNF to a DNF using de Morgan rules (just distribute $\wedge$ over $\vee$), for example
\[
...
\begin{enumerate}
\item 
 An \textit{assignment} $\rho$ for $l$ variables in $\bar{x}$ is  a function $\rho: \bar{x} \to \{0,1, *\}$, where $\rho(x_i) = *$ means that $x_i$ is \textit{unassigned} (or ``free'') by $\rho$.
...
The Switching Lemma thus shows that the ``bad'' event of a function computed by an $r$-DNF formula not being able to ``switch'' to a function that can be computed by an $s$-CNF, is small. This would mean that with high enough probability the ``good'' event happens: a random assignment switches an $r$-DNF to an $s$-CNF.
We note that this would \emph{not} be enough to conclude the argument. We shall also need to make sure that such a ``good'' random assignment exists  moreover \emph{leaves many variables free}. For this purpose, we shall use a common tool in probabilistic analysis and concrete complexity: \emph{concentration bounds} (in our case it will be Chernoff Bound; see below).
...
Recall we want to prove \Cref{thm:ACZ-lower bound} that stating that every circuit family of depth $d$ computing the PARITY function requires size $2^{\Omega\left(n^{\nicefrac{1}{d-1}}\right)}$.

Consider the bottom two levels of the given AC\textsuperscript{0} circuit. Since we assume that the levels of AC\textsuperscript{0} circuit always alternate between ANDs and ORs, the depth-2 sub-circuits at these bottom two levels are all either CNFs or DNFs. To apply the Switching Lemma to them, we need to make them into $k$-CNFs (or $k$-DNFs) first, for some \emph{small} $k$, since the formulation of the lemma assumes $k$ to be ... But why? what is the precise k t we need? We shall need it to be small for the probabilistic computation: namely, we need to bound the probability of failure, to be able to apply the union bound.  
