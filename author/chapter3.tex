...
A \emph{bad} restriction here means a restriction $\rho$ for which \( T(f\rst\rho) \) has depth \( > s \). 
    ...
    The code for $\rho\sigma$  and its helper code are built based on the following  idea: consider \( \rho \in S \), and let \( \pi \) be the first (leftmost) path in \( T(f \restriction \rho) \) exceeding length \( s \). 
...
\begin{figure}[H]
\includegraphics[width=\textwidth]{images/coding-sl-ex1-b.png}
...
To explain this figure and the coding procedure in general we use the following notation:
\begin{itemize}
...
We thus have  \( \pi' := \pi_1' \pi_2' \dots \pi_k' \), where the number of possible codes for \( \pi' \) is \( 2^s \). 
\end{itemize}
