...
\begin{figure}[H] 
    ...
    \caption{Example of a $k-1$-colouring that fools an approximator that checks for a clique on  the set $X_i$.}
    \label{fig:R(xi)}
\end{figure}
...
\\
\intertext{
since $\frac{n-i}{k-i} \geq \frac{n-l}{k}$
...
We prove the base approximator case of  both lemmas together, as the argument is the same.  

\para{Base Approximator Case} $|C|=1$, i.e., $C$ consists of only a single input gate $g_{ij}$. Recall that $g_{i j}$ is an input gate denoting whether $(i, j) \in E$, for $i, j \in V$.
...
\para{Induction Approximator Step }
...
\begin{definition}[Approximate OR and AND]
