...
Up to this point we have  seen \textbf{circuit complexity}, in which we wanted to understand the resources required to \emph{compute} a function. And specifically, what is the minimal size of a Boolean circuit computing a given function (as a function of the number of input bits).

We now consider \textbf{proof complexity}. Instead of analysing the size of circuits computing Boolean
functions, we address the following question:
...
\hangindent=2em
\hangafter=0
{\it Given a propositional proof system, and a tautological
...
Similar to circuit complexity which is motivated by $\P$ vs $\NP$ (if SAT
does not have small circuits then $\P \neq \NP$), proof
complexity is motivated by the $\NP$ vs $\coNP$ question as we shall see.
...
\textbf{\NP}: the class of  languages whose positive instances have polynomial-size witnesses. Namely, languages for which a positive input can be verified in polynomial-time. \\\\
\textbf{\coNP}: the class of  languages whose \textit{negative} instances have
polynomial-size witnesses.
Namely, languages for which a positive input may lack a short witness that can be efficiently verified, but a negative input does have an easily verifiable short witness.\\
...
For concreteness, we can think of $\NP$ and $\coNP$ as their representative complete problem:
\begin{align*}
    SAT \text{ as }  \NP \\
    UNSAT \text{ as } \coNP.
\end{align*}
Indeed  SAT is \NP-complete, hence, UNSAT is \coNP-complete.
 

\begin{itemize}
    \item For a language $L$ in NP, we know we have short (polynomial-size) proofs for every $x$ in $L$: if $L = SAT$, then the short proof is simply the satisfying assignment.
    \item For language $L$ in coNP, we don't know if $x$ in $L$ have short proofs. If $L = UNSAT$, and $x$ is in $L$, then it is unclear what kind of short proofs are there for unsatisfiability? In fact, it is open if there are.
...
For the rest of this chapter we focus on a  concrete proof system, which is well studied, simple enough to describe and investigate, and quite well-understood. 


\begin{tcolorbox}[colframe=white, colback=red!5, boxrule=0mm, sharp corners]
\begin{definition}[Resolution Refutations]
Let $F$ be a set of  clauses $C_1, \dots, C_m$. A \textit{resolution derivation of the clause $D$ from $F$}, in symbols $F\vdash D$,  is a sequence of clauses $D_1, \dots, D_t$ such that the final clause $D_t = D$ 
%(the empty clause, which stands for the truth-value %\emph{false}), 
and every $D_i$, $i=1,\dots,t$, is one of the following:
\begin{itemize}
\item 
$D_i$ is an initial clause $C_j$ for some $j=1,\dots,m$, or
\item $D_i$ was derived from $D_j, D_k$ with $j,k < i$ via the following resolution derivation rule:
...
\begin{center}
    ...
    \end{tabular}
\end{center}
where $D_i=A\lor B$ and $A, B$ are some (possibly empty) clauses.
\item 
$D_i$ was derived by the following rule: 
...
\begin{center}
    ...
    \end{tabular}
\end{center}
where $D_i=A\lor l$, for $l$ some literal and $A$\ some possibly empty clause. 
\end{itemize}

The \textbf{size} of a resolution derivation is the \emph{number of clauses} it has.

A \textbf{resolution refutation} of the CNF formula $F$ is a derivation of the  empty clause $\square$ (which stands for the truth-value \emph{false}).
\end{definition}
\end{tcolorbox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% \end{itemize}
 ...
 The example depicted a \textbf{tree-like} resolution refutation because the underlying structure is a \textbf{tree}. Formally, this means that every clause in the refutation is used at most once in a resolution rule (to derive a new clause). 

A resolution refutation that is \textbf{not} a tree, namely, the same occurrence of a clause can be used in more than one resolution-rule or weakening-rule applications, is called a \textbf{DAG-like refutation}. Because the refutation can be drawn as a \textbf{Directed Acyclic Graph}.


\begin{remark}
We use the word "proof" and "refutation" interchangeably in proof complexity.
\end{remark}

Note that a refutation of a CNF formula is a proof that the CNF formula  is unsatisfiable. Or in other words, a proof that the negation of the CNF is a tautology, i.e., evaluates to 1 under every assignment.

\begin{exercise}
Prove that if a CNF formula $F$ has a resolution refutation then $F$ is unsatisfiable. 
(Hint: assume by way of contradiction the existence of a satisfying assignment, and use induction on refutation  length to get a contradiction, using the fact that $\square$ has no satisfying assignments.)
\end{exercise}
...
\begin{definition}[Restriction of a  refutation]
Let $F$ be a CNF formula.
Define $F{\rst}_{x_i = \epsilon}$, $~\epsilon \in \{0,1\}$ as follows:
For all clauses $C$ in $F$ do:
\begin{enumerate}
    \item If $C$ is a clause in $F$ and $x_i^\epsilon \in C$, then delete $C$ from $F$ (because the clause is true under the assignment, it does not contribute to unsatisfiability).
    \item If $x_i^{1-\epsilon} \in C$, then delete the literal $x_i^{1-\epsilon}$ from $C$, but keep $C$ (because, $C\lor x_i^{1-\eps}\equiv C$; note that $C$ can be the empty clause $\square $). 
...
\begin{proof}
Let $F$ be a CNF formula that is unsatisfiable. This means that $F$ evaluates to false (or 0) under every truth assignment to the variables of $F$. Our goal is to show that the empty clause $\square$ can be derived using resolution.
...
\induction  Assume that resolution is complete for CNFs with $n$ variables.
    ...
    \item We perform a ``splitting argument'' by considering:
    \begin{itemize}
        \item $F\rst_{x_{n+1}=1}$, the simplified formula when $x$ is set to true.
        \item $F\rst_{x_{n+1}=0}$, the simplified formula when $x$ is set to false. 

Since $F$ is unsatisfiable, both $F\rst_{x_{n+1}=1}$ and $F\rst_{x_{n+1}=0}$ must also be unsatisfiable.
        \item By the induction hypothesis, there exist resolution refutations for both. We can \emph{glue} these refutations into one, as is shown in the Gluing Lemma below eventually leading to the empty clause $\square$.
    \end{itemize}
...
\end{lemma}
This suffices for our proof of completeness theorem above, when we put $i = n+1$.

\begin{tcolorbox}[colframe=white, colback=blue!5, boxrule=0mm, sharp corners]
\para{Upshot of the argument}

We construct from a refutation of $F\!\rst_{x_i=0}$ a derivation of $x_i$ from $F$. We then use this derivation, namely the clause $x_i$, to ``cut'' (using the resolution rule) all occurrences of $\neg x_i$ from $F$. By cutting $\neg x_i$ from $F$ we are able to obtain  $F\!\rst_{x_i=1}$, because $F\!\rst_{x_i=1}$ contains precisely all clauses $C$ such that $\neg x_i\lor C$ is in $F$ and all clauses $D$ that neither contain $x_i$ nor $\neg x_i$ (clauses $x_i\lor E$ in $F$ are discarded from $F$ when we assign $x_i=1$). By induction hypothesis we know that  $F\!\rst_{x_i=1}$ has a resolution refutation and we are finished.
\end{tcolorbox}
...
We show that we can build a derivation $F \vdash x_i$, meaning a resolution derivation of $x_i$ from $F$.
We are going to turn the refutation from $F_{x_i = 0}$ to a \emph{derivation} of $x_i$ from $F$.
  Intuitively, this is done by adding OR of $x_i$ to each clause in the refutation $F_{x_i = 0} \vdash \square$.
How do we do this precisely?  
  

Consider the three types of clauses in $F$:
...
Now consider what happens to $ F_1, F_2, F_3$ in $F_{x_i := 0}$:
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item So in $F_{x_i := 0}$, this clause becomes \textit{True} and is deleted.
    \end{itemize}
...
\begin{enumerate}
    \item   A clause $D \lor x_i$ in $F_1$ turned into the clause $D$ in $F_{x_i := 0}$.  
Now, when we add $\lor x_i$, we get back to the original clause $D \lor x_i$ in $F$.
    ...
    \item A clause $D$ in $F_3$ does not contain $x_i$ or $\neg x_i$. When we add $\lor x_i$ to $D$, we get a new clause not in $F$. But we can derive, using a single \textbf{Weakening Rule}, the clause $D \lor x_i$.  
Hence, in every place $D \lor x_i$ appears, we simply add $D$ and a weakening rule application to derive $D\lor x_i$ to the resolution derivation, which turn it into a legal resolution derivation from $F$.

\item 
 If in $F_{x_i := 0} \vdash \square$ we used the resolution rule:
    \[
...
\item Finally, adding $x_i\lor$ to $\square$ turns it into $x_i$. Which concludes the claim.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{An illustration of the gluing lemma. }
    \label{fig:gluing-lemma}
\end{figure}

By the above, we get a resolution derivation $\pi$ of $x_i$ from $F$.
 

- Now, we use this derivation  $F \vdash x_i$ on the clauses of $F$ to get rid (resolve, i.e., use the resolution rule to cut) all the literals $\neg x_i$ in $F$.
...
- Note that this way we got a derivation of $F_{x_i := 1}$ from $F$.
  ...
  This is because $F_{x_i := 1}$ contains precisely all the clauses in $F$ that contain neither $x_i$ nor $\neg x_i$ ($F_3$ above), and the clauses $D$ such that $\neg x_i \lor D$ is in $F$ (i.e., $F_2$ above).  
  (The clause $ x_i \lor D$ in $F_1$ gets deleted in $F_{x_i := 1}$.)
\medskip 
 
By assumption $F_{x_i := 1} \vdash \square$, namely, after we derived $F_{x_i := 1}$ from $F$ with a resolution derivation, we derive $\square$, and finish the proof of the gluing lemma.
...
\section{Size-Width Relations for Resolution}
 ...
 We show here that the existence of short resolution refutations for a CNF formulas means that there is a resolution refutation of this formula that has small width, namely a refutation in which each clause has small amount of literals. This enables to lower bound the size of resolution refutations via lower bounding the width of resolution refutations  (since, a short refutation would imply a small width refutation, contradicting the width lower bound).

\subsection*{Notations:}
\begin{enumerate}
    \item The \emph{size} (equivalently, length) of a resolution refutation is the number of clauses in it.
...
The idea is to  recurse by partitioning the tree-like refutation into two \emph{disjoint} sub-refutations, of which one is at most $s/2$. Thus, we partition the derivation of $\Box$ into two disjoint parts, the first deriving $x_i$ and the second $\neg x_i$, for some $i$ (note this must always occur in a refutation: it would always end with this resolution rule application, except for the trivial case in which  the CNF formula already contain the empty clause). Since one of these derivations is of size at most $s/2$ we can now use induction on the logarithm of $s$: if we denote $b=\log s$ then $\log\ s/2 =b-1$. 
\bigskip 


We prove that:
\begin{equation}
\label{eq:gluing-lemma-1-to-prove}
...
Proceed by \emph{double} induction on the number of variables $n$ \emph{and} $b$.

\textbf{Base case 1:}  
 $b = 0$. Thus, $S_T(F \vdash \square) \leq 2^0 = 1$. So the refutation is just $\square$.  
Noting that the width of $\square$ is 0, we get    
\[
w(F \vdash \square) = 0 \leq \log_2(1) + w(F).
\]

\textbf{Base case 2:} If $n = 1$, then the only possible refutation is:
...
\end{array}~,
\]
and so $w(F) = 1$ and the size of the refutation is 3, hence  
\[
w(F \vdash \square) = 1  \leq \log_2(3) + 1.
\]

\textbf{Induction step:}  
We assume that  \Cref{eq:gluing-lemma-1-to-prove} holds for $b-1$ and $n-1$ and prove it for $b$ and $n$.

Consider a tree-like refutation of $F$ whose size is at most $2^b$. As mentioned before, every (non trivial) refutation must end with the following rule application, for some variables $x_i$:
...
Without loss of generality, we assume that $\Gamma$ on the left is of size $\leq 2^{b-1}$ (as it is impossible that both sides are larger than $2^{b-1}$). Then, by induction hypothesis (on $b$):
\[
w(F|_{x_i := 0} \vdash \square) \leq \log_2(S_T(F|_{x_i := 0} \vdash \square)) + w(F) = b - 1 + w(F).
\]

And by induction on $n$:

\[
w(F_{x_i := 1} \vdash \square) \leq \log_2(S_T(F_{x_i := 1} \vdash \square)) + w(F) \leq b+w(F).
\]

It suffices now to glue the two proofs together in a way that preserved small width.
This is shown below via inspecting that the gluing lemma we proved in \Cref{lem:glueing-no-width} indeed preserves small width.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{The Gluing Lemma Preserves Small Width}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The following gluing lemma is identical to the gluing lemma above, only that we inspect every step and observe that the depth in each step does not increase above $b$, when we glue together a $(b-1)$-width refutation  with a $b$-width  refutation.
...
\begin{proof}
We start with the following claim that is identical to the claim above, only that we make sure the width  is preserved at most $b$.
\begin{claim}
 $\leq b - 1 + w(F)$. Then, $F \vdash_{b + w(F)} x_i$, meaning there is a resolution derivation of $x_i$ from $F$ of width $\leq b + w(F)$.

\end{claim}

\begin{proof}[Proof of claim]
By inspection of the claim in the proof of \Cref{lem:glueing-no-width}.
...
By adding $\lor x_i$ to each clause in $F_{x_i := 0} \vdash_{b - 1 + w(F)} \square$ we are able  to convert  
\[
...
F \vdash_{b + w(F)} x_i
\]  
(i.e., a derivation of the clause $x_i$ from $F$, of width $b + w(F)$), as we explain below.
\begin{enumerate}
...
\begin{enumerate}
    \item[] If in $F_{x_i := 0} \vdash \square$, we used the \textbf{resolution rule}:
    \[
...
This is simple: derive $x_i$ from $F$, and then resolve $D \lor \neg x_i$ in $F$  
by $x_i$ to get $D$.
Note that this way we get a derivation of $F_{x_i := 1}$ from $F$, of width $b + w(F)$.  
...
we derive $\square$, and finish the proof of the lemma.
\end{proof}
...
Since $|\pi^*|\le|\pi|$ we have
    ...
    \]
    We use the following simple approximation for $0<\varepsilon<1$:
    \[
    ...
    \]
We use another simple approximation:
\[
...
Here we describe a different way to obtain resolution refutation size lower bounds via the notion of \emph{Feasible Interpolation}. 
...
\label{thm:logical-interpolant-thm}
Let \(\varphi(\overline{x},\overline{z})\) be a propositional formula over variables \(x_{1},\dots,x_{n}, z_{1},\dots,z_{m}\), and let \(\psi(\overline{y},\overline{z})\) be a propositional formula over variables \(y_{1},\dots,y_{\ell}, z_{1},\dots,z_{m}\). 
Suppose that the only shared variables between \(\varphi\) and \(\psi\) are \(\overline{z}\). 
Assume that the formula 
\[
\varphi(\overline{x},\overline{z}) \;\land\; \psi(\overline{y},\overline{z})
\]
is unsatisfiable.
...
\begin{equation}\label{eq:1178}
\bigl(\varphi(\overline{x},\overline{z}) \;\land\; I(\overline{z})\bigr)
\;\;\lor\;\;
\bigl(\psi(\overline{y},\overline{z}) \;\land\;\neg I(\overline{z})\bigr)
\end{equation}
is unsatisfiable.
\end{theorem}
% 
...
% 
% \includegraphics[scale=0.8]{images/image020.png}
% \includegraphics[scale=0.5]{images/image019.png}



\begin{proof}
For every assignment \(\overline{c} \in \{0,1\}^m\), either \(\varphi(\overline{x}, \overline{c})\) is unsatisfiable or \(\psi(\overline{y}, \overline{c})\) is unsatisfiable (or both are unsatisfiable). 
This is because if both \(\varphi(\overline{x},\overline{c})\) and \(\psi(\overline{y},\overline{c})\) are satisfiable, then we could combine these two assignments for \(\overline{x}\) and \(\overline{y}\) (noting that \(\overline{x}\) and \(\overline{y}\) are disjoint) to obtain a satisfying assignment for \(\varphi \wedge \psi\). Hence \(\varphi \wedge \psi\) would not be unsatisfiable, contradicting our assumption.

\begin{enumerate}[label=\textbf{(\alph*)}]
    \item \textbf{Case 1:} \(\varphi(\overline{x},\overline{c})\) is satisfiable, which implies \(\psi(\overline{y},\overline{c})\) must be unsatisfiable. 

    We define \( I(\overline{c}) := 0\). Then observe that
    \[
      \bigl(\underbrace{\overbrace{\varphi(\overline{x},\overline{c})}^{\rm satisfiable} \;\land\; \underbrace{I(\overline{c})}_{0}}_0\bigr)
      \;\;\lor\;\;
      \bigl(\overbrace{\psi(\overline{y},\overline{c}) }^{\rm unsatisfiable}\;\land\;\underbrace{\neg I(\overline{c})}_1\bigr)
    \]
    is unsatisfiable. 
    %(1)

    \item \textbf{Case 2:} Otherwise, \(\varphi(\overline{x},\overline{c})\) is unsatisfiable. We then set \(I(\overline{c}) := 1\). Hence
    \[
      \bigl(\varphi(\overline{x},\overline{c}) \;\land\; I(\overline{c})\bigr)
      \;\;\lor\;\;
      \bigl(\psi(\overline{y},\overline{c}) \;\land\;\neg I(\overline{c})\bigr)
    \]
    is unsatisfiable.
...
The following corollary stems immediately from the fact that  formula \eqref{eq:1178} is unsatisfiable.
\begin{corollary} \label{cor:simple-interpolation-cor}
The interpolant in \Cref{thm:logical-interpolant-thm} has the following properties:

 If \(I_n(\overline{z}) = 1\), then \(\varphi_n(\overline{x},\overline{z})\) is unsatisfiable;

 If \(I_n(\overline{z}) = 0\), then \(\psi_n(\overline{y},\overline{z})\) is unsatisfiable.
\end{corollary}

\begin{theorem}
[Feasible Interpolation Theorem for Resolution]
\label{thm:interpolation-circuit}
In the setting of the previous theorem, suppose 
\(\varphi(\overline{x},\overline{z}) \wedge \psi(\overline{y},\overline{z})\) 
...
Furthermore, if the variables \(\overline{z}\) appear all in \emph{positive} form in 
\(\psi(\overline{y},\overline{z})\), then the above circuit is \emph{monotone}. 
Similarly, if all \(\overline{x}\) variables in \(\varphi(\overline{x},\overline{z})\) 
appear in \emph{negative} form, the resulting circuit is also monotone.
\end{theorem}

Applying this theorem to derive a resolution refutation lower bound is now quite easy to attain, as we can argue that any short resolution refutation would yield a small circuit for a function known to hard for monotone circuits, leading to a contradiction.
...
Let us define the following unsatisfiable CNF formulas \(\varphi_n(\overline{x},\overline{z})\) and \(\psi_n(\overline{y},\overline{z})\). Intuitively, let
\[
\varphi_n(\overline{x},\overline{z}) = 1 \quad\text{iff}\quad \overline{x} \text{ encodes a }\sqrt[4]{n}\text{-clique in the graph on }n\text{ nodes represented by }\overline{z}.
\]
\[
\psi_n(\overline{y},\overline{z}) = 1 \quad\text{iff}\quad \overline{y} \text{ encodes a }(\sqrt[4]{n}-1)\text{-colouring of that same graph represented by $\overline{z}$.}
\]
Then,
\[
\varphi_n(\overline{x},\overline{z}) \;\wedge\; \psi_n(\overline{y},\overline{z})
\]
...
$ \varphi_n(\overline{x},\overline{z}) \;\wedge\; \psi_n(\overline{y},\overline{z})$ formally, with a CNF of  \(O(n^2)\) size, ensuring that all \(\overline{z}\) variables appear \emph{positively} in \(\varphi_n\) and \emph{negatively} in \(\psi_n\).
...
two integers. Then the graph $G$ cannot be an \emph{extreme graph} as in \Cref{sec:Razborov}, which is simultaneously a positive and negative input to the ${\rm Clique}(n,k)$ problem. In other words, one of the following must hold:
\begin{description}
...
Namely, there is no way to partition $G$ into $k'$ subgraphs
$G_1\dots G_{k'}$,
such that each $G_i$ is an independent set, and for all $i\ne j \in[k']$,
...
\underline{$\psi_n(\overline{y},\overline{z})$}:
...
By the feasible interpolation \Cref{thm:interpolation-circuit}, if there exists a resolution refutation of 
\(\varphi_n(\overline{x},\overline{z}) \;\wedge\; \psi_n(\overline{y},\overline{z})\)
...
\(\textsc{CLIQUE}(n,\sqrt[4]{n})\), thus contradicting the monotone circuit lower bound we proved in \Cref{sec:Razborov}!

\medskip

\noindent
\textbf{Why does \(I_n\) compute \(\textsc{CLIQUE}(n,\sqrt{n})\)?}
Consider the extreme positive or negative input graphs to \(\overline{z}\). By \Cref{cor:simple-interpolation-cor}:
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item If \(I_n(\overline{z}) = 1\), then \(\varphi_n(\overline{x},\overline{z})\) is unsatisfiable;
    \item If \(I_n(\overline{z}) = 0\), then \(\psi_n(\overline{y},\overline{z})\) is unsatisfiable.
\end{enumerate}

Thus, for \(\overline{z}\in\bits^n\) that encodes either a \(\sqrt[4]{n}\)-clique or a \((\sqrt[4]{n}-1)\)-colouring in the underlying graph, namely, $\overline{z}$ comes from the class of extreme graphs, we have the following:
\[
I_n(\overline{z}) = 
\begin{cases}
1, & \text{if }\overline{z}\text{ encodes a }(\sqrt[4]{n}-1)\text{-colouring},\\
0, & \text{if }\overline{z}\text{ encodes a }\sqrt[4]{n}\text{-clique}.
\end{cases}
\]
Hence \(I_n\) distinguishes between \(\sqrt[4]{n}\)-cliques and \((\sqrt[4]{n}-1)\)-colourings, which is exactly the function we showed in \Cref{sec:Razborov} to require size $2^{\Omega(\sqrt[8]{n})}$. 
\end{proof}
...
Given an assignment \(\overline{c} \in \{0,1\}^m\), we explain how to compute \(I(\overline{c})\) by transforming a size-\(s\) resolution refutation of
\[
\varphi(\overline{x},\overline{z}) \;\wedge\; \psi(\overline{y},\overline{z})
\]
into a refutation of either \(\varphi(\overline{x}, \overline{c})\) (meaning \(\varphi(\overline{x}, \overline{c})\) is unsatisfiable, so \(I(\overline{c}) = 1\)) or \(\psi(\overline{y}, \overline{c})\) (meaning \(I(\overline{c}) = 0\)). We accomplish this in time \(O(s^2)\).
...
The resulting refutation has size \(O(s)\). Concretely, we take the original resolution refutation and, using the assignment \(\overline{c}\), \emph{discard} all \(\overline{z}\) variables from every clause in the proof. By doing so, we obtain a refutation of
\[
\varphi(\overline{x},\overline{c}) \;\wedge\; \psi(\overline{y},\overline{c}).\]
...
\begin{enumerate}
\item 
  contains \emph{only} \(\overline{x}\)-variables or \emph{only} \(\overline{y}\)-variables; and 
...
\]
where \(\widetilde C_i\) is the version of \(C_i\) with all \(\overline{z}\) variables removed, leaving either only \(\overline{x}\) or only \(\overline{y}\).

\paragraph{Conclusion}
Since the final line \(\widetilde C_m\) in the refutation is false (denoted by \(\Box\)), and by induction statement is derived correctly from previous clauses, we derive a contradiction in either \(\varphi(\overline{x}, \overline{c})\) or \(\psi(\overline{y}, \overline{c})\). 
...
\noindent
\textbf{Base Case:} 
Every axiom clause in \(\varphi(\overline{x}, \overline{c})\) or \(\psi(\overline{y}, \overline{c})\) is already an \(\overline{x}\)-clause or a \(\overline{y}\)-clause (i.e., it contains only \(\overline{x}\) variables or only \(\overline{y}\) variables, because $\overline c$ is a constant assignment).
...
D' \;=\; \neg w \;\lor\; D
\]
by a resolution rule application.
...
\medskip
Note that the last line  of the original resolution refutation is \(\Box\) (i.e.\ false). Hence in our new refutation (possibly ending earlier than the last clause of the original refutation), we must also reach \(\Box\). Starting from the axioms, we obtain a refutation of either \(\varphi(\overline{x},\overline{c})\) or \(\psi(\overline{y},\overline{c})\). 
Thus, the proof concludes with the desired contradiction in one of these formulas. 


\bigskip 

To prove that the same algorithm can be implemented with a \emph{monotone} circuit in case the $\overline z$-variables all appear positive in $\psi$ or they all appear negative in $\varphi$, we notice the following:\ For every index \(j\), let \(d_j(\overline{c}) = 0\) if the \(j\)-th clause \(\widetilde{C_j}\) in the modified refutation can be derived using only the \(\overline{y}\) variables. It is possible to show that 
 each \(d_j(\overline{c})\) can be computed by a monotone circuit of size \(O(s^2)\) in the variables \(\overline{c}\).

Observe that \(I(\overline{c}) = d_m(\overline{c})\), namely $I(\overline{c})$ is zero only if  $\psi(\overline y,\overline{c})$ is unsatisfiable, and otherwise it is 1, as desired. 
\end{proof}
