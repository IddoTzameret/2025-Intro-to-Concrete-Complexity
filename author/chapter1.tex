...
\begin{description}
\item[Time bounds for Turing Machines (TMs):] counts the maximal number of steps a specific Turing Machine $M$ is doing when input with a string of  size $n$. 

\item[The class \P:]  Polynomial time computation (also denoted  PTIME, or polytime, or p-time).

\item[The class \NP, verifiers, short certificates:] Nondeterministic Polynomial-time computation. Namely, the class of languages that can be decided by a nondeterministic Turing Machine in time bounded by a polynomial in the input length.  That is, if the input is of size $n$ the running time of the nondeterministic Turing machine should not exceed $n^c$, for some constant $n$ independent of $n$ (e.g.,  $c=40$). 
...
\item[SAT is \NP-complete (Cook-Levin theorem)] See e.g., \cite{AB09,Pap94}.
\end{description}
...
\begin{trailer}{Answer}\label{trailer:xor-circuit}
