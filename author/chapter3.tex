...
\begin{itemize}
    ...
    \item \textbf{Note:} If we know \( \rho\sigma \), it does not \emph{a priori} mean we can recover \( \rho \), because a different \( \rho' \) can be mapped to \( \rho'\sigma' = \rho\sigma \) (\emph{the domain of 0/1 assignment in \( \rho \) and \( \rho' \) and $\sigma$ and $\sigma'$ may be different}).
    ...
    \textcolor{gray}{(We shall prune this path to length \( s \) precisely).}
\end{itemize}
...
\begin{itemize}
    ...
    \item This means we are able to distinguish between \( \rho\sigma \) and \( \rho \) because variables assigned by $\rho$ cannot be then re-assigned in the tree $T(f{\rst}\rho)$, hence recover \( \rho \) \emph{(uniquely)}.
\end{itemize}
\end{tcolorbox}
