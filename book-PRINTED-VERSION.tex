%%%%%%%%%%%%%%%%%%%% book.tex %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% sample root file for the chapters of your "monograph"
%
% Use this file as a template for your own input.
%
%%%%%%%%%%%%%%%% Springer-Verlag %%%%%%%%%%%%%%%%%%%%%%%%%%


% RECOMMENDED %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[graybox,envcountchap,sectrefs]{style/svmono}

% choose options for [] as required from the list
% in the Reference Guide

%\usepackage{mathptmx}
%\usepackage{helvet}
%\usepackage{courier}
%
\usepackage{type1cm}         
\usepackage{makeidx}         % allows index generation
\usepackage{graphicx}        % standard LaTeX graphics tool
                             % when including figure files
\usepackage{multicol}        % used for the two-column index
\usepackage[bottom]{footmisc}% places footnotes at page bottom

\usepackage{newtxtext}       % 
\usepackage{newtxmath}       % selects Times Roman as basic font
\usepackage{hyperref}       % 


\usepackage{style/basicmathmacros}
\usepackage[sanserif,full]{style/complexity}
\usepackage{cleveref}
\usepackage{nicefrac}
\usepackage[most]{tcolorbox} % for frramed colored boxes 
\usepackage{enumitem} % Include the enumitem package this for the [resume] option in enumerate lists 

% see the list of further useful packages
% in the Reference Guide

\makeindex             % used for the subject index
                       % please use the style svind.ist with
                       % your makeindex program

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\usepackage{float} % force image locations
\graphicspath{{./}{images/}}
\usepackage{color}
\usepackage[normalem]{ulem}
\usepackage{draftwatermark}
\usepackage{tikz}
\usetikzlibrary{positioning,circuits.logic.US, shapes.geometric, positioning}
\usepackage{proof} % for use with large vdash 

% Set initial watermark properties
\SetWatermarkText{DRAFT} % No watermark initially
\SetWatermarkScale{1.} % Adjust size
\SetWatermarkLightness{0.99} % Adjust lightness
\SetWatermarkAngle{45} % Set angle of the watermark

%%%%%%%%%%%%%%%%
% \usepackage{background}
% \backgroundsetup{
%   position=current page.north,
%   angle=0,
%   nodeanchor=north,
%   vshift=-5mm,
%   opacity=0.5,
%   scale=1,
%   contents={\color{gray}Draft; comments and suggestions welcome}
% }



%%%%%%%%%%%%%%%%%%%%
\usepackage{amsbsy}
\usepackage{amsmath}

% ------------------
% My claim environment
%\newtheorem{myclaim}{Claim}[section]





\begin{document}

\author{Iddo Tzameret}
%\title{Introduction to Concrete Complexity}
\title{Introduction to Complexity Lower Bounds}
\subtitle{Lecture Notes \\ 
\normalsize{Imperial College London} \\ \normalsize{Dept. of Computing}
}

\maketitle


\frontmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%\include{author/dedication}
%\include{author/foreword}
%\include{author/preface}
%\include{author/acknowledgement}


\tableofcontents

%\include{author/acronym}


\mainmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\include{author/part}
\include{author/chapter1}
\include{author/chapter2}
\include{author/chapter3}
\include{author/chapter4}
\include{author/acknowledgement}

\backmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\include{author/glossary}
%\include{author/solutions}
%\printindex

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



% Disable watermark again at this point
\SetWatermarkText{}

\bibliographystyle{plain}
\bibliography{PrfCmplx-Bakoma}

\end{document}





 
