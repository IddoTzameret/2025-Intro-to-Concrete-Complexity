...
\textit{Input}: Undirected graph $G=(V,E)$ with $n$ nodes (encoded as above), and a number $k$ (given in unary, i.e., $1^k$).
...
\begin{tcolorbox}[float=t, colframe=white, colback=blue!5, boxrule=0mm, sharp corners]
...
The upshot is the following: \emph{by a way of contradiction,} consider a purported small monotone circuit computing  $\operatorname{CLIQUE}({n}, {k})$, and show by induction, gradually progressing from the input nodes towards the output node, that the function computed at every gate can be approximated by a small crude circuit. Then, the output gate is also approximated by a small crude circuit. Now, use an auxiliary argument to show that there is no small crude circuit approximating $\operatorname{CLIQUE}({n}, {k})$.
...
\section{Proof of monotone circuit lower bounds \Cref{thm:razborov}}

 \begin{tcolorbox}[float=h, colframe=white, colback=blue!4, boxrule=0mm, sharp corners]
...
The approximation of the monotone circuit $C$ that computes $\operatorname{CLIQUE}(n, k)$ is done by induction on the size of $C$, i.e., number of $\lor, \wedge$ gates in $C$.

\begin{note} Such an induction is usually called \emph{induction on the structure of $C$}, since we are going to induce on subcircuits of $C$, assuming the induction hypothesis holds for them, and using the structure of the circuit, conclude that the induction hypothesis also holds for the gate that connects those two subcircuits.
\end{note}

This structural induction proceeds as follows. 
...
This is an easy case: We need to show a crude circuit $\operatorname{CC}\left(X_1, \ldots, X_\right)$ with $m \leq M$ and $\left|X_i\right| \leqslant l$, for all $i \in[m]$ that approximates $g_{i j}$ (without introducing too many errors; we shall count precisely the number of potential errors later).
But the circuit $C C(\{i, j\})$ is $g_{i j}$ by definition. (Hence, no errors here!)


\subsubsection{Induction Step of the proof}
Given two crude circuits
$\operatorname{CC}(\mathcal X)$ and 
$\operatorname{CC}(\mathcal Y) 
$, with $\mathcal{X}=\{X_1, \ldots, X_m\}$, 
$\mathcal{Y}=\{Y_1, \ldots, Y_{m^{\prime}}\}$, $m\le M$, and $\left|X_i\right| \leq \ell $, for all $i$, $m'\le M $ and $\left|Y_i\right| \leq \ell$, for all $i$, we wish to construct another crude circuits  for computing $CC(\mathcal{X}) \vee CC(\mathcal{Y})$, and $\operatorname{CC}(\mathcal X) \wedge 
...
\subsection{The Sunflower Lemma}
...
\subsection{Approximating OR and AND using Plucking}
